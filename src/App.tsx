import { Play, SkipBack, SkipForward, Repeat, Shuffle, Home, Search, Library, Plus, Heart } from 'lucide-react';
            ...
            <aside className="w-64 bg-black flex flex-col gap-2 p-2 shrink-0">
                ...
                <div className="bg-zinc-900 rounded-lg p-4 flex-1 flex flex-col gap-4 overflow-hidden">
                    ...
                    <div className="flex flex-col gap-2 overflow-y-auto pr-2">
                        {[1, 2, 3, 4, 5].map((item) => (
                            <div key={item} className="flex items-center gap-3 p-2 hover:bg-zinc-800 rounded-md transition cursor-pointer group">
            ...
            <main className="flex-1 bg-gradient-to-b from-zinc-800 to-zinc-900 m-2 ml-0 rounded-lg overflow-y-auto">
                ...
                <section className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                            <div key={i} className="flex items-center gap-4 bg-zinc-800/50 hover:bg-zinc-700/50 transition rounded cursor-pointer group overflow-hidden">
                ...
                <section className="p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-2xl font-bold hover:underline cursor-pointer">Sizin için derlendi</h2>
                        <span className="text-xs font-bold text-zinc-400 hover:underline cursor-pointer uppercase tracking-widest">Hepsini Gör</span>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {[1, 2, 3, 4, 5].map((i) => (
                            <div key={i} className="p-4 bg-zinc-900/40 hover:bg-zinc-800/60 transition rounded-lg cursor-pointer group">
                                ...
                                <p className="text-sm text-zinc-400 line-clamp-2 mt-1">Sizin için hazırlanan özel çalma listesi.</p>
                            </div>
                        ))}
                    </div>
                </section>
