...
function ProfileDetailView({ userId }: ProfileDetailViewProps) {
  ...
  const userPublicPlaylists = data.playlists.filter(
    (playlist) => playlist.owner_id === user.id && playlist.public,
  );

  const preferredArtists = (monthlyTopArtistIdsByUser[user.id] ?? [])
    .map((artistId) => data.artists.find((artist) => artist.id === artistId))
    .filter((artist): artist is Artist => Boolean(artist));

  const fallbackArtists = data.artists.slice().sort((a, b) => b.popularity - a.popularity);
  const topArtists = [...preferredArtists, ...fallbackArtists]
  ...
  const followingText = t('profileDetail.following', {
    count: formatNumber(followingByUser[user.id] ?? 0, locale) as unknown as number,
  });
            ...
            {userPublicPlaylists.map((playlist) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-1 line-clamp-2 text-sm text-zinc-400">By {user.display_name}</p>
                </Link>
              );
            })}
          </div>
            ...
            {data.users
              ...
              .map((follower) => (
                ...
                >
                  ...
                  <p className="text-sm text-zinc-400">{t('profileDetail.profile') || 'Profile'}</p>
                </Link>
              ))}
          </div>
        </section>
